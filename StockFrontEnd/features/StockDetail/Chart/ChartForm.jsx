                        ...
                        <button className="tw3v-emtxt715 tw3v-emtxt7p tw3v-emtxt7t tw3v-emtxt710" aria-disabled="false" aria-label="차트모양" data-theme="grey" data-variant="clear" data-mode="dark" data-state="closed">
                            <span className="tw3v-17xiat90" aria-hidden="false" role="presentation" style={{ height: "14px", width: "14px", minWidth: "14px", color: "var(--wts-adaptive-grey700)" }}>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="">
                                    ...
                                    <path fill="none" stroke="#fc2d4c" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" d="M20 3.98v9.93M12 3.18v18.44"></path>
...
function ChartTimeConfig() {
    const isOpen = useChartButtonStore((state) => state.isResolutionOpen);
    const toggleMinute = useChartButtonStore((state) => state.toggleResolution);
    const selectedMinute = useChartButtonStore((state) => state.selectedMinute);
    const selectedChartTime = useChartButtonStore((state) => state.selectedChartTime);
    const setChartTime = useChartButtonStore((state) => state.setChartTime);

    const isMinuteActive = Object.keys(ChartMinuteEnum).includes(selectedChartTime);
    ...
    return (
        <div className="_1owxq237">
            <button
                ...
                className={
                    isMinuteActive
                        ? "tw3v-1wkoka52h tw3v-1wkoka58 tw3v-1wkoka541 tw3v-1wkoka5e tw3v-1wkoka517 tw3v-1wkoka5x tw3v-1wkoka5r tw3v-1wkoka5l tw3v-1wkoka528"
                        : "tw3v-1wkoka52h tw3v-1wkoka5a tw3v-1wkoka5e tw3v-1wkoka517 tw3v-1wkoka5x tw3v-1wkoka5r tw3v-1wkoka5l tw3v-1wkoka528"
                }
                data-tds-wts-button
                id="radix-_r_ign_"
                aria-haspopup="menu"
            ...
            >
                <span className="tw3v-1wkoka52g">{selectedMinute}분</span>

                <span
                    style={{
                        display: "inline-flex",
                        alignItems: "center",
                        color: "var(--wts-adaptive-grey700)"
                    }}
                >
            ...
            {Object.entries(ChartTimeEnum).map(([key, label]) => (
                <button
                    onClick={() => setChartTime(label)}
                    key={key}
                    type="button"
                    aria-disabled="false"
                    className={
                        selectedChartTime === key
                            ? "tw3v-1wkoka52h tw3v-1wkoka59 tw3v-1wkoka5e tw3v-1wkoka517 tw3v-1wkoka5x tw3v-1wkoka5r tw3v-1wkoka5l tw3v-1wkoka528"
                            : "tw3v-1wkoka52h tw3v-1wkoka5a tw3v-1wkoka5e tw3v-1wkoka517 tw3v-1wkoka5x tw3v-1wkoka5r tw3v-1wkoka5l tw3v-1wkoka528"
                    }
                    data-tds-wts-button
                >
